...
export class StateTransitionService {
  ...
  private canCancelGame(game: Game): boolean {
    return game.status === GameStatus.WAITING || game.status === GameStatus.PLAYING;
  }
  ...
  private async onEnterQuestionBuild(game: Game): Promise<void> {
    // Update game metadata to indicate question building phase
    game.metadata = {
      ...game.metadata,
  ...
  private async updateGameState(game: Game, newState: GameState): Promise<Game> {
    // Update game metadata with new state
    game.metadata = {
      ...game.metadata,
